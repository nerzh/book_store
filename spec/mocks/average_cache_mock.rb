...
class AverageCacheMock < ActiveMocker::Base
  created_with('2.0.0')
...
#_class_methods.erb
  class << self
    def attributes
      @attributes ||= HashWithIndifferentAccess.new({"id"=>nil, "rater_id"=>nil, "rateable_id"=>nil, "rateable_type"=>nil, "avg"=>nil, "created_at"=>nil, "updated_at"=>nil}).merge(super)
    end

    def types
      @types ||= ActiveMocker::HashProcess.new({ id: Fixnum, rater_id: Fixnum, rateable_id: Fixnum, rateable_type: String, avg: Float, created_at: DateTime, updated_at: DateTime }, method(:build_type)).merge(super)
    end
    ...
    def attribute_names
      @attribute_names ||= ["id", "rater_id", "rateable_id", "rateable_type", "avg", "created_at", "updated_at"] | super
    end
...
# _scopes.erb
  ...
  class ScopeRelation < ActiveMocker::Association
    include AverageCacheMock::Scopes
  end

  def self.__new_relation__(collection)
    AverageCacheMock::ScopeRelation.new(collection)
  end
